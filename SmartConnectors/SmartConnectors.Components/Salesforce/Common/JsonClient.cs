...
namespace SmartConnectors.Components.Salesforce.Common
{
    public class JsonClient : IDisposable
    {
        ...
        public async Task<T> HttpPatchAsync<T>(object inputObject, Uri uri, Dictionary<string, string> customHeaders = null, bool deserializeResponse = true, bool serializeRequest = false)
        {
            try
            {
                HttpContent content = new StringContent(JsonConvert.SerializeObject(inputObject), Encoding.UTF8, "application/json");
