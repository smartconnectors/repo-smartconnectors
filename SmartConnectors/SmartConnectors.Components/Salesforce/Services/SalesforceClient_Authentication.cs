...
{
    public class SalesforceClient_Authentication
    {
        private const string UserAgent = "forcedotcom-soap-toolkit-dotnet";
        private HttpClient _httpClient;

        public SalesforceClient_Authentication()
        {
            _httpClient = new HttpClient();
        }

        public SalesforceClient_Authentication(HttpClient httpClient)
        {
        ...
        {
            var url = "https://login.salesforce.com/services/Soap/u/29.0/" + model.OrgId;
            var soap = string.Format(@"
                                    <soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"">
                                    <soapenv:Body>
                                    <login xmlns=""urn:partner.soap.sforce.com"">
                                        <username>{0}</username>
                                        <password>{1}{2}</password>
                                    </login>
                                    </soapenv:Body>
                                    </soapenv:Envelope>", model.Username, model.Password, model.SecurityToken);
        ...
        private async Task<string> Create(string query, string sessionId, string metadataServerUrl)
        {
            ...
            var soap = string.Format(
                                ...
                                wsdlNamespace, sessionId, header, action, wsdlNamespace, query, action);
